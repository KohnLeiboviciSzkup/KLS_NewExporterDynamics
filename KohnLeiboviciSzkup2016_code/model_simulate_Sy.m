function [r sim] = model_simulate_Sy(m,s,r)
    ...
    for t=1:s.T*(1+s.burn)  
        ...
        % Update state variables    
            ...
            if s.Tshocks==1
                ...
                %Transition from high to lower trade cost
                    temp1 = sim.tauRND(t+1,:)<=m.Trho; %Switching from high to low
                    temp2 = r.exporter(ind)==1; %Export                    
                    temp3 = ind_y(t,:)~=s.y_grid_size; %Lowest cost
                    ind_y(t+1,logical(temp1.*temp2.*temp3)) = ind_y(t,logical(temp1.*temp2.*temp3)) + 1; %1=high, 2=low  

                %Exporters with lowest cost, stay with lowest cost
                    temp1 = sim.tauRND(t+1,:)<=m.Trho; %Switching from high to low
                    temp4 = ind_y(t,:)==s.y_grid_size; %2;
                    ind_y(t+1,logical(temp1.*temp4)) = s.y_grid_size; %2;                    
                    
                %No Transition from high to lower trade cost
                    temp1 = sim.tauRND(t+1,:)>m.Trho; %Switching from high to low
                    temp2 = r.exporter(ind)==1; %Export                    
                    ind_y(t+1,logical(temp1.*temp2)) = ind_y(t,logical(temp1.*temp2)); %1=high, 2=low                      
...
% Labor             
     ...
     sim.n_f_TC = r.n_f_export_TC(ind).*exporter_decision;
...
% Sales     
    ...
    temp = export_intensity(xhistory==1);
    sim.export_sales_ratio_initial_med = median(temp(:));
    
    temp = prctile(export_intensity(xhistory>=1),75);
    sim.export_sales_ratio_p75 = median(temp(:));
    ...
    % Labor       
            n_f_TC = zeros(s.T,s.N);
            n_f_TC(exporter_decision==0) = 0; 
            n_f_TC(exporter_decision==1) = r.n_f_export_TC(ind(exporter_decision==1)); 
            
            n_d = zeros(s.T,s.N);
            n_d(exporter_decision==0) = r.n_d(ind(exporter_decision==0));
            n_d(exporter_decision==1) = r.n_d_export(ind(exporter_decision==1));
            
            temp1 = n_f_TC(xhistory>=1) + n_d(xhistory>=1);            
            temp2 = n_d(xhistory<=-1);
    ...
    % Labor
            ...
            temp1 = n_f_TC(xhistory>1) + n_d(xhistory>1);            
            temp2 = n_f_TC(xhistory==1) + n_d(xhistory==1);
            sim.cont_export_size_premium_med = median(temp1(:))/median(temp2(:));  
            clear temp1 temp2 exporter_decision  
...
%Sales statistics   
    ...
    if s.Dshocks==1        
       sim.salesf_std =  std(log(sales_f(xhistory>=1)));
       sim.salesd_std =  std(log(sales_d(xhistory>=1)));
       sim.salesFD_SDratio = sim.salesf_std/sim.salesd_std;
    end
...
%% State variables -> y shocks and export status (whether the firm exported or not in the previous period)

% Initialize objects
    sim.exporter = zeros(s.T*(1+s.burn)+1,s.N);   

    
% Compute y-shocks and export states for t>1    
    for t=1:s.T*(1+s.burn)  
        ...
        if s.Tshocks==1
           ...
           % Exporters with high cost: Some transition from high to low trade cost in the following period
           % Which firms?
           %(1) Those that exported in the previous period
           %(2) Those that had a high cost
           %(3) Those that get a draw to switch from high to low     
           %Transition from high to lower trade cost
                temp1 = sim.tauRND(t+1,:)<=m.Trho; %Switching from high to low
                temp2 = r.exporter(ind)==1; %Export
...
% Labor
     ...
     sim.n_f_TC = r.n_f_export_TC(ind).*sim.exporter_decision;
...
% Productivity and extra shock   

    sim.z = r.z_grid(ind_z); 
    sim.y = r.y_grid(ind_y);
    
     

%% Variables, firm-level statistics, and aggregate statistics

 % Test: volatility across time periods of average productivity and average assets    
...
% Firm-level statistics
    ...
    % Exporter size premium -> exporters vs non-exporters
        % Labor            
            n_f_TC(sim.exporter_decision==0) = 0; 
            n_f_TC(sim.exporter_decision==1) = r.n_f_export_TC(ind(sim.exporter_decision==1)); 
            n_d(sim.exporter_decision==0) = r.n_d(ind(sim.exporter_decision==0));
            n_d(sim.exporter_decision==1) = r.n_d_export(ind(sim.exporter_decision==1));
            
            temp1 = n_f_TC(sim.xhistory>=1) + n_d(sim.xhistory>=1);            
            temp2 = n_d(sim.xhistory<=-1);
            sim.export_size_premium_med = median(temp1(:))/median(temp2(:));  
            sim.export_size_premium_mean = mean(temp1(:))/mean(temp2(:)); 
            
            sim.testF2 = median(n_f_TC(sim.xhistory>=1));
            sim.testD2 = (n_d(sim.xhistory>=1));
            sim.testD2 = median(sim.testD2(:));         
    ...
    % Continuing exporter size premium -> continuing exporters vs new exporters
        %Labor
            temp1 = n_f_TC(sim.xhistory>1) + n_d(sim.xhistory>1);            
            temp2 = n_f_TC(sim.xhistory==1) + n_d(sim.xhistory==1);
            sim.cont_export_size_premium_med = median(temp1(:))/median(temp2(:));  
            sim.cont_export_size_premium_mean = mean(temp1(:))/mean(temp2(:)); 
            clear temp1 temp2 
    ...
    % New exporter size premium -> new exporters vs non exporters
        %Labor
            ...
            sim.testF = median(n_f_TC(sim.xhistory==1));
            sim.testD = (n_d(sim.xhistory==1));
            sim.testD = median(sim.testD(:));
            
            sim.new_export_size_premium_med = median(temp1(:))/median(temp2(:));   
            sim.new_export_size_premium_mean = mean(temp1(:))/mean(temp2(:));
            clear temp1 temp2         
...
for i = 1:s.CN 
    %Average
        %Level
            ...
            %sim.xperiods_const_in(i,1) = mean(sim.const_in(sim.xperiods_xhistory==i));
            %sim.xperiods_const_in_profits(i,1) = mean(sim.const_in_profits(sim.xperiods_xhistory==i));             
...
%Sales statistics   
    ...
    if s.Dshocks==1        
       sim.salesf_std =  std(log(sim.sales_f(sim.xhistory>=1)));
       sim.salesd_std =  std(log(sim.sales_d(sim.xhistory>=1)));
       sim.salesFD_SDratio = sim.salesf_std/sim.salesd_std;
    end    
